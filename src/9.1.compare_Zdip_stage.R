...
unique(all_control_med_VT$plant)
# Replace "C1" and "C2" with "C1_C2", and "BD1" and "BD2" with "BD1_BD2"
all_control_med_VT <- all_control_med_VT %>%
  dplyr::mutate(plant = dplyr::case_when(
    plant %in% c("C1", "C2") ~ "C1_C2",
    plant %in% c("BD1", "BD2") ~ "BD1_BD2",
    TRUE ~ plant
  ))
...
# Replace "C1" and "C2" with "C1_C2", and "BD1" and "BD2" with "BD1_BD2"
all_control_med_R1 <- all_control_med_R1 %>%
  dplyr::mutate(plant = dplyr::case_when(
    plant %in% c("C1", "C2") ~ "C1_C2",
    plant %in% c("BD1", "BD2") ~ "BD1_BD2",
    TRUE ~ plant
...
# Replace "C1" and "C2" with "C1_C2", and "BD1" and "BD2" with "BD1_BD2"
all_control_med_R4 <- all_control_med_R4 %>%
  dplyr::mutate(plant = dplyr::case_when(
    plant %in% c("C1", "C2") ~ "C1_C2",
    plant %in% c("BD1", "BD2") ~ "BD1_BD2",
    TRUE ~ plant
...
# Replace "C1" and "C2" with "C1_C2", and "BD1" and "BD2" with "BD1_BD2"
all_control_high_VT <- all_control_high_VT %>%
  dplyr::mutate(plant = dplyr::case_when(
    plant %in% c("C1", "C2") ~ "C1_C2",
    plant %in% c("BD1", "BD2") ~ "BD1_BD2",
    TRUE ~ plant
...
# Replace "C1" and "C2" with "C1_C2", and "BD1" and "BD2" with "BD1_BD2"
all_control_high_R1 <- all_control_high_R1 %>%
  dplyr::mutate(plant = dplyr::case_when(
    plant %in% c("C1", "C2") ~ "C1_C2",
    plant %in% c("BD1", "BD2") ~ "BD1_BD2",
    TRUE ~ plant
...
# Replace "C1" and "C2" with "C1_C2", and "BD1" and "BD2" with "BD1_BD2"
all_control_high_R4 <- all_control_high_R4 %>%
  dplyr::mutate(plant = dplyr::case_when(
    plant %in% c("C1", "C2") ~ "C1_C2",
    plant %in% c("BD1", "BD2") ~ "BD1_BD2",
    TRUE ~ plant
...
########## DESINKED
all_desinked_med_VT <- rbind(predicted_N_zdip_med_VT_e1_AD1,
                             predicted_N_zdip_med_VT_e1_AD2,
                             predicted_N_zdip_med_VT_e1_AD,
                             predicted_N_zdip_med_VT_e2_AD1,
                             predicted_N_zdip_med_VT_e2_AD2,
                             predicted_N_zdip_med_VT_e2_AD,
                             predicted_N_zdip_med_VT_e3_AD1,
                             predicted_N_zdip_med_VT_e3_AD2,
                             predicted_N_zdip_med_VT_e3_AD)

all_desinked_high_VT <- rbind(predicted_N_zdip_high_VT_e1_AD1,
                              predicted_N_zdip_high_VT_e1_AD2,
                              predicted_N_zdip_high_VT_e1_AD,
                              predicted_N_zdip_high_VT_e2_AD1,
                              predicted_N_zdip_high_VT_e2_AD2,
                              predicted_N_zdip_high_VT_e2_AD,
                              predicted_N_zdip_high_VT_e3_AD1,
                              predicted_N_zdip_high_VT_e3_AD2,
                              predicted_N_zdip_high_VT_e3_AD)
...
# Replace "A1" and "A2" with "A1_A2"
all_desinked_med_R1 <- all_desinked_med_R1 %>%
  dplyr::mutate(plant = dplyr::case_when(
    plant %in% c("AD1", "AD2") ~ "AD1_AD2",
    TRUE ~ plant
  ))
...
# Replace "A1" and "A2" with "A1_A2"
all_desinked_high_R1 <- all_desinked_high_R1 %>%
  dplyr::mutate(plant = dplyr::case_when(
    plant %in% c("AD1", "AD2") ~ "AD1_AD2",
    TRUE ~ plant
  ))
...
# Replace "A1" and "A2" with "A1_A2"
all_desinked_med_R4 <- all_desinked_med_R4 %>%
  dplyr::mutate(plant = dplyr::case_when(
    plant %in% c("AD1", "AD2") ~ "AD1_AD2",
    TRUE ~ plant
  ))
...
# Replace "A1" and "A2" with "A1_A2"
all_desinked_high_R4 <- all_desinked_high_R4 %>%
  dplyr::mutate(plant = dplyr::case_when(
    plant %in% c("AD1", "A2") ~ "AD1_A2",
    TRUE ~ plant
  ))
...
unique(all_control_high_R4$plant)
unique(all_control_high_R4$leaf)
...
for (stage in stages) {
  for (treatment in treatments) {
    ...
    dataset_name <- paste0("all_control_", treatment, "_", stage)
    
    # Use get() to dynamically access the dataset
    data_filtered <- get(dataset_name)
    
    # Calculate the average predicted_N by old_genotype, plant, and leaf
    genotype_plant_summary <- data_filtered %>%
      group_by(old_genotype, plant, leaf) %>%
      summarise(
    ...
    print(genotype_plant_summary)
    
    # Save the summary as a CSV file
    output_filename <- paste0("genotype_plant_summary_Control_", treatment, "_", stage, ".csv")
    write.csv(genotype_plant_summary, output_filename, row.names = FALSE)
  }
}
...
for (stage in stages) {
  for (treatment in treatments) {
    # Construct the variable name for the dataset
    dataset_name <- paste0("all_desinked_", treatment, "_", stage)
    ...
    # Save the summary as a CSV file
    output_filename <- paste0("genotype_plant_summary_desinked_", treatment, "_", stage, ".csv")
    write.csv(genotype_plant_summary, output_filename, row.names = FALSE)
  }
}
...
# Save the summary as a CSV file
#write.csv(genotype_plant_summary_high_R4, "genotype_plant_summary_high_R4.csv", row.names = FALSE)
...
for (stage in stages) {
  for (treatment in treatments) {
    ...
    data_filtered <- all_control_med %>%
      filter(stage == stage, N_treatment == treatment)
    
    # Calculate the average based on plant and leaf
    plant_summary <- data_filtered %>%
      group_by(plant, leaf) %>%
      summarise(
        mean_predicted_N = mean(predicted_N, na.rm = TRUE),
        .groups = 'drop'
    ...
    # Print the summary for each combination
    print(paste("Summary for stage:", stage, "and treatment:", treatment))
    print(plant_summary)
    
    # Save the summary as a CSV file
    write.csv(plant_summary, paste0("plant_summary_", treatment, "_", stage, ".csv"), row.names = FALSE)
  }
}
